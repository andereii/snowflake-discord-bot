...
export async function execute(interaction) {
    ...
    const borrarDias = interaction.options.getInteger('delete_days') || 0;
    ...
    try {
        ...
        if (member) {
            ...
            await notifyMemberDm(member, 'Veto', motivo);
        }

        await interaction.guild.members.ban(usuario.id, { 
            deleteMessageSeconds: borrarDias * 24 * 60 * 60, 
            reason: motivo 
        });

        const incidente = registerIncident(guildId, usuario, interaction.user, IncidentType.Veto, motivo);
        await announceIncident(interaction.guild, incidente);

        const exito = MessagesService.get(guildId, 'Moderacion:Exito:Veto', { usuario: usuario.username });
