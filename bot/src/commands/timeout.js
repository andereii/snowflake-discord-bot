...
export const data = new SlashCommandBuilder()
    .setName('timeout')
    .setDescription('Timeout (isolate) a user')
...
export async function execute(interaction) {
    ...
    try {
        ...
        await notifyMemberDm(member, 'Aislamiento', motivo, { duracion: duracionStr });
        await member.timeout(durationMs, motivo);

        const incidente = registerIncident(guildId, usuario, interaction.user, IncidentType.Aislamiento, motivo, duracionStr);
        await announceIncident(interaction.guild, incidente);

        const exito = MessagesService.get(guildId, 'Moderacion:Exito:Aislamiento', { usuario: usuario.username, duracion: duracionStr });
